...
def test_add_row_udfs(tmpdir):
    ...
    sdf = rlu.Month().apply_to(sdf)

    sdf = rlu.Year().apply_to(sdf)

    sdf = rlu.WaterYear().apply_to(sdf)

    sdf = rlu.NormalizedFlow().apply_to(sdf)

    sdf = rlu.Seasons().apply_to(sdf)
...
def test_add_timeseries_udfs(tmpdir):
    ...
    ped = tau.PercentileEventDetection()
...
def test_add_udfs_write(tmpdir):
    ...
    ped = tau.PercentileEventDetection()
    ev.joined_timeseries.add_udf_columns(ped).write()
...
def test_location_event_detection(tmpdir):
    ...
    ped = tau.PercentileEventDetection()
    sdf = ev.metrics.add_udf_columns(ped).query(
