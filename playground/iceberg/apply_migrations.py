...
def read_available_schema_versions(
  ...
  """
  schema_versions: list[int] = [int(d) for d in os.listdir(f'{catalog_name}/schemas')]
...
def create_schema_evolution_support(
  ...
  """
  Creates the necessary schema evolution metadata tables in the specified catalog.
...
def update_applied_schema_version(
  ...
  """
  Updates the applied schema version for a given catalog.
...
def load_schema_version_evolution_statements(
  ...
  version_dir_name = f'{catalog_name}/schemas/{schema_version:04}'
...
def evolve_catalog_schema(
  ...
  """
  Evolves a catalog schema by applying any new schema versions
