...
class Fetch:
    ...
    def nwm_operational_points(
        ...
        nwm_version : SupportedNWMOperationalVersionsEnum
            ...
            Availability of each version. A switchover lands on a forecast
            cycle rather than at midnight, and the requested date range is
            validated at that resolution:
            ...
            These are NOAA's intended boundaries, not a promise about every
            file: configurations do not all switch on the same cycle, so a
            file still reporting the outgoing version within a day of a
            boundary is accepted with a warning.
        start_date : Union[str, datetime, pd.Timestamp]
            Date and time to begin data ingest.
            Str formats can include YYYY-MM-DD HH:MM or MM/DD/YYYY HH:MM.
