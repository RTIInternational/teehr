...
export default function MetricSelect(props) {
   ...
   const { metrics, setMetrics, selectedMetrics, setSelectedMetrics, selectedDataset } = props;
   ...
   useEffect(() => {
      const fetchMetricFields = () => {
         if (selectedDataset && selectedDataset !== "") {
            axios
               .get(`http://localhost:8000/datasets/${selectedDataset}/get_metric_fields`)
               .then((res) => {
         ...
         {!loading ? (
            !errors ? (
               <FormControl sx={{ m: 1, display: 'flex' }}>
                  <InputLabel id="demo-multiple-chip-label">Select Metrics</InputLabel>
                  <Select
                     ...
                     input={<OutlinedInput id="select-multiple-chip" label="Selected Metrics" />}
